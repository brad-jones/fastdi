...
- Keep CI pipelines as thin orchestration layers that delegate to Taskfile tasks and Python scripts
...
Chosen option: "Keep CI pipelines as thin orchestration layers that delegate to Taskfile tasks and Python scripts",
...
- Good, because any pipeline step can be run locally with `task <name>` or `uv run scripts/<name>.py`.
- Good, because migrating to another CI platform requires only a thin new config file — not a rewrite.
- Good, because pipeline logic is type-checked and testable (Python scripts) or clearly structured (Taskfile).
...
2. A call to `task <name>` or `uv run scripts/<name>.py`.
...
- Good, because Python scripts are self-contained, testable, and version-controlled like application code.
