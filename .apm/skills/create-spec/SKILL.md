...
- **`README.md`** — the spec itself, in English: what this capability is, the public API it introduces (classes,
  ...
  couple of worked examples in prose. This is what a human or LLM reads to understand *what* to build; it is the
...
- **`pyproject.toml`** — a `uv` workspace member:
  ...
  The root `pyproject.toml`'s `[tool.uv.workspace] members = ["specs/*"]` picks this up automatically — no other
  registration needed.
